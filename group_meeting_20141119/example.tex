...
\begin{frame}{Inference Problem}
	\begin{itemize}
		...
		\begin{align*}
			\vy_{\Tcal}(\vx) = \underset{\vy\in\vYcal}{\argmax}\,\sum_{t=1}^{n}\ip{\vw_{T_t}}{\phi_{T_t}(\vx_i,\vy_i)},
		\end{align*}
		\item The inference problem on an individual spanning tree can be solve efficiently by \textit{dynamic programming}
		\begin{align*}
			\vy_{T_t}(\vx) = \underset{\vy\in\vYcal}{\argmax}\,\ip{\vw_{T_t}}{\phi_{T_t}(\vx_i,\vy_i)}.
		\end{align*}
		\item There is no guarantee $\vy_{\Tcal}=\vy_{T_t},\forall T_t\in\Tcal$
	\end{itemize}
\end{frame}
