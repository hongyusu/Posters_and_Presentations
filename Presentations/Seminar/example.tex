...
\begin{frame}{So far}
	\begin{itemize}\footnotesize
		\item We work with multilabel classification problems in general.
		\item In addition, we assume there is an output graph given {\em apriori}.
		\item We develop structured output prediction method which model the label correlations with an output graph.
		\item If the output graph is tree-like, the inference problem can be solved exactly with a polynomial time algorithm, e.g., dynamic programming.
		\item If the output graph is a general graph structure, the inference problem is \nphard and can be solved with approximation algorithm, e.g., loopy belief propagation.
		\item The next question is what if the output graph is not observed.
	\end{itemize}
\end{frame}
%
...
\begin{frame}[allowframebreaks]{Fast inference over a collection of trees}
	\begin{itemize}
		...
		\end{align*}
		\item Performing the same computation on all trees gives a candidate list of $n\times K$ multilabels in $\Theta(nK\ell)$ time
		\begin{align*}
