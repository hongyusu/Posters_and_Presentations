...
\begin{frame}{Output graph is directed}{Predicting network response}
	...
	\only<6>{David does not like soccer.  \includegraphics[scale=0.5]{./figures/motivation_6.pdf}}
\end{frame}
...
%
\begin{frame}{So far in sides}
	\begin{itemize}\footnotesize
		\item We develop structured output prediction model which model the label correlation with an output graph. 
		\begin{tabular}{|c|c|c|}
			\hline
			\footnotesize
			 Output graph & Inference problem & Inference algorithm \\ \hline
	...
	\begin{itemize}
		...
		\begin{tabular}{|c|c|c|}
			...
			\footnotesize
			 ...
			 {\bf unknown} & \nphard & \rta\ \cite{su14multilabelnips} \\ \hline
		\end{tabular}
	\end{itemize}
\end{frame}
...
\begin{frame}{Conclusion}
	\begin{itemize}\footnotesize
		...
		\item The output graph is often assume to known {\em apriori}.
		\begin{itemize}\footnotesize
