...
\begin{frame}{A sample of spanning trees}
	\begin{itemize}
		...
		\end{align*}
		\item When $n\ge\frac{\ell^2}{\epsilon^2}(\frac{1}{16}+\frac{1}{2}\ln\frac{8\sqrt{n}}{\delta})$, with high probability, we have $|F_{\Tcal}(\vw,\vx,\vy)-F(\vw,\vx,\vy)|\le\epsilon$.
		\item A sample of $n\in\Theta(\ell^2/\delta^2)$ random spanning tree is sufficient to estimate the score function.
		\item Margin achieved by the complete graph is also preserved by the sample of $n$ random spanning trees.
	\end{itemize}
\end{frame}
