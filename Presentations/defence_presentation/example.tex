...
\begin{frame}{Single label classification}
	\begin{itemize}
		...
		\begin{itemize}
			\footnotesize
			\item Input is an \textit{\color{aaltoblue} object} (e.g., an image).
			\item Output is an attribute called \textit{\color{aaltoblue} label} (e.g., dog or cat).
			...
			\item Learn a \textit{\color{aaltoblue}mapping function} that predicts the label of a new object.
		\end{itemize}
		\item Mathematically, we define the single label classification problem
		\begin{itemize}
			\footnotesize
			\item Data comes in pairs $(\vx,y)\in\vXcal\times\Ycal$, $\vXcal=\RR^d$ is an input space, $\Ycal=\{+1,-1\}$ is an output space.
			\item We are given a set of training data $\Scal=\{(\vx_i,y_i)\}_{i=1}^{m}$.
			\item Learn a mapping function $f\in\Hcal$ that predict the best output of an input $\vx\overset{f}{\rightarrow}y$.
		\end{itemize}
...
\begin{frame}{Image annotation task}
	\begin{itemize}
		...
		\{boat, sea, sun, beach, people, dog, cat, $\cdots$\}
...
\begin{frame}{Multilabel classification}
	\begin{itemize}
		\item The problem is known as \textit{\color{aaltoblue}multilabel classification}, which is a natural extension to single label classification.
		\begin{itemize}
			\footnotesize
			\item Data comes in pairs $(\vx,\vy)\in\vXcal\times\vYcal$, $\vXcal=\RR^d$ is an input space, $\vYcal=\Ycal_1\times\cdots\times\Ycal_k, \Ycal_j = \{+1,-1\}$ is an output space.
			\item We are given a set of training data $\Scal=\{(\vx_i,\vy_i)\}_{i=1}^{m}$.
			\item Learn a mapping function $f\in\Hcal$ that predict the best output of an input $\vx\overset{f}{\rightarrow}\vy = \{y_1,\cdots,y_k\}$.
		\end{itemize}
	\end{itemize}
\end{frame}
